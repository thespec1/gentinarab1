...
(function ($) {
    ...
    function menuResponsive() {
        ...
        if ($('.navigation_right .menu-sub-menu-container').length) {
            if (screenWidth < nbfoody.menu_resp) {
    ...
    if ($upsellsCells.length <= nbfoody.upsells_columns) {
    ...
    if ($relatedCells.length <= nbfoody.related_columns) {
    ...
    if ($crossSellsCells.length <= nbfoody.cross_sells_columns) {
	...
	if ($('.related .swiper-container').length){
		var slidesm = 2;
		var slidemd = 3;
		if(nbfoody.related_columns==2){
			slidesm = 1;
			slidemd = 1;
		}
		var related = new Swiper('.related .swiper-container', {
			slidesPerView: nbfoody.related_columns,
	...
	if ($('.upsells .swiper-container').length){
		var slidesm = 2;
		var slidemd = 3;
		if(nbfoody.upsells_columns==2){
			slidesm = 1;
			slidemd = 1;
		}
		var upsells = new Swiper('.upsells .swiper-container', {
			slidesPerView: nbfoody.upsells_columns,
			pagination: '.swiper-pagination',
			paginationClickable: true,
			breakpoints: {
				991: {
	...
	if ($('.cross-sells .swiper-container').length){
		var slidemd = 3;
		var slidelg = 4;
		if(nbfoody.cross_sells_columns==3){
			slidemd = 2;
			slidelg = 3;
		}
		var crossSells = new Swiper('.cross-sells .swiper-container', {
			slidesPerView: nbfoody.cross_sells_columns,
			pagination: '.swiper-pagination',
			paginationClickable: true,
			breakpoints: {
				1199: {
	...
	var swiperInit = function() {
        if ($('.featured-gallery').length && $('.thumb-gallery').length){
            ...
            if(nbfoody.thumb_pos !== 'right-dots') {
                ...
                if(nbfoody.thumb_pos === 'left-thumb' || nbfoody.thumb_pos === 'inside-thumb') {
    ...
    $.fn.yith_quick_view = function() {
        ...
        $(document).off( 'click', '.yith-wcqv-button' ).on( 'click', '.yith-wcqv-button', function(e){
            ...
            t.block({
                message: null,
                overlayCSS  : {
                    background: '#fff url(' + nbfoody.loader + ') no-repeat center',
                    opacity   : 0.5,
                    cursor    : 'none'
                }
            });
